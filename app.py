    ...
    with col_right:
        ...
        with st.expander("📐 단위셀 ↔ 전체구조 Cd 차이는 왜 생기나 (물리적 원인)",
            ...
            st.markdown(
                ...
                "**정량 검증 (U=1.0, 동일 그물·조건, CFD 직접 비교):**\n\n"
                "| 영각 | Cd 단위셀 | Cd 전체구조 | 차이 | 해석 |\n"
                "|---|---|---|---|---|\n"
                "| **90° (정면)** | 1.108 | 1.061 | **4.2%** | 거의 일치 — 파이프라인 검증 |\n"
                "| 45° (사각) | 0.600 | 0.779 | 30% | 유한 그물의 유동방향 압력항 |\n\n"
                "**정면 충돌에서 4%로 일치**(둘 다 Cl≈0)하는 것은 좌표·기준면적·정규화·"
                "솔버가 올바름을 입증합니다. 사각에서 벌어지는 이유는, 무한 스크린(단위셀)은 "
                "힘이 **순수 법선**이라 유동방향 투영이 줄지만, 유한 그물(전체구조)은 전·후면 "
                "정체·가장자리 박리로 **유동방향 압력항**이 더해지기 때문입니다. Aref 비는 "
                "9.17≈9셀로 정규화는 일관하며, 이 사각 차이는 **유한 vs 무한**·**개방 vs 주기** "
                "경계조건의 물리적 차이라 수렴·격자를 높여도 완전히 사라지지 않습니다.\n\n"
                "**활용 가이드:** 무한 균일 그물의 대표 계수가 필요하면 *단위 셀*을, "
                "특정 크기 그물망의 실제 항력·가장자리 효과가 필요하면 *전체 구조*를 "
                "사용하세요. 둘은 상호 보완 관계이며 절대값 일치를 목표로 하지 않습니다.")
