...
## 6. 초정밀 후속 실험 (P1~P4, 2026-07-03)

교차점 미커버 27% 해소·스택 강화를 위한 4개 실험 — **전부 챔피언 미달**,
현행 설정(4층·첫층3e-5·exp1.3·fa60·ftr0.5·nGrow0·relaxed)이 국소 최적으로 확정.

| 실험 | 변경 | 결과 | 판정 |
|---|---|---|---|
| P1 | 표면 distance +1레벨(0.156mm) | snappy **FPE** (hexRef8·AMI 정합성) | 구조적 차단 |
| P2′ | 6층·첫층 0.02mm·normals3 | 커버 **19.3%**·Cd 0.741 | 스택↑=커버↓ |
| P3 | 제트박스 lvl3(AMI 2mm 회피) | snappy **FPE** (전이버퍼가 AMI 도달) | 구조적 차단 |
| P4 | ftr1.0·iter80·fa45·nGrow1 | 커버 **2.2%**·Cd 0.773 | 역효과 |

### 구조적 한계 2건 (향후 과제의 전제)
1. **cyclicAMI + hexRef8**: AMI 상태에서 쉘 정밀화 레벨을 올리면(직접이든
   전이버퍼 경유든) `consistentSlowRefinement2` FPE. 관통 와이어가 AMI 평면
   위의 반원기둥이라 정합성 검사가 깨짐. → 우회하려면 '평면 patch 로 메싱 후
   createPatch 로 cyclic 재결합' 경로나 cfMesh 등 대체 메셔 필요.
2. **교차점 레이어 절단 연쇄**: 스택을 어느 방향으로든 키우면(층수·노브)
   truncation 연쇄로 커버리지 급락. 73% 가 이 조합의 상한.

### 커버리지–Cd 상관 (실측)
0% → 0.681 · 2% → 0.77 · 19% → 0.741 · **73% → 1.073** (단조 경향,
100% 외삽 ≈ 1.15~1.25 — 벤치마크 1.257·이론 하단과 정합).
잔여 격차(이론 1.43)는 커버리지 27% + 준2D/정상해 한계 복합.
